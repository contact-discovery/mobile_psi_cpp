...
int main(int argc, char** argv) {
    ...
    std::thread server([]{
        ...
        droidCrypto::KosDotExtSender sender;
        sender.setBaseOts(baseOT, choizes);
        sender.setDelta(droidCrypto::AllOneBlock);

        std::vector<std::array<droidCrypto::block,2>> mesBuf(NUM_DOTE);
...
//    }
    ...
    auto time2 = std::chrono::high_resolution_clock::now();
    droidCrypto::KosDotExtReceiver recv;
    recv.setBaseOts(baseOT);

    droidCrypto::BitVector choizes(NUM_DOTE);
    choizes.randomize(p);
    std::vector<droidCrypto::block> mesBuf(NUM_DOTE);
    ...
    auto time3 = std::chrono::high_resolution_clock::now();
    std::vector<std::array<droidCrypto::block,2>> buf(NUM_DOTE);
    for(size_t i = 0; i < NUM_DOTE; i++) {
        chan.recv(buf[i][0]);
        chan.recv(buf[i][1]);
