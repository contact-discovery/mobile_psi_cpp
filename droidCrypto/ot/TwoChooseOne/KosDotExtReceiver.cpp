...
void KosDotExtReceiver::receive(const BitVector &choices,
  ...
  for (uint64_t blockIdx = 0; blockIdx < bb; ++blockIdx) {
    ...
    for (; dd < stop0; ++dd, ++i) {
      ...
      Utils::mul256(msg[dd][0], msg[dd][1], challenges[i], challenges2[i], ti1,
                    ti2, ti3, ti4);
      t[0] = t[0] ^ ti1;
      t[1] = t[1] ^ ti2;
      t[2] = t[2] ^ ti3;
  ...
  chl.send((uint8_t *)correlationData.data(),
           correlationData.size() * correlationData[0].size() * sizeof(block));
