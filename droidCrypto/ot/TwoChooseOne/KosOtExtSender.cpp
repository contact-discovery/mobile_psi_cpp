...
void KosOtExtSender::setBaseOts(span<block> baseRecvOts,
...
void KosOtExtSender::send(span<std::array<block, 2>> messages, PRNG &prng,
                          ChannelWrapper &chl) {
  // round up
  ...
  for (uint64_t superBlkIdx = 0; superBlkIdx < numSuperBlocks; ++superBlkIdx) {
    ...
    while (mIter != mEnd) {
      while (mIter != mEnd && tIter < tEnd) {
        ...
        // std::cout << "midx " << (mIter - messages.data()) << "   tIdx " <<
        // tIdx << std::endl;
    ...
    while (xIter != xEnd) {
      while (xIter != xEnd && tIter < tEnd) {
        *xIter = *tIter;
        ...
        // uint64_t tV = tIter - (block*)t.data();
        // uint64_t tIdx = tV / 8 + (tV % 8) * 128;
        // std::cout << "xidx " << (xIter - extraBlocks.data()) << "   tIdx " <<
        // tIdx << std::endl;
  ...
  for (uint64_t i = 0; i < 128; ++i) {
    if (neq(xtraBlk[i], choices[i] ? extraBlocks[i] ^ delta : extraBlocks[i])) {
      ...
      std::cout << extraBlocks[i] << "  " << (extraBlocks[i] ^ delta)
                << std::endl;
  ...
  for (uint64_t blockIdx = 0; blockIdx < bb; ++blockIdx) {
    ...
    for (uint64_t i = 0, dd = doneIdx; dd < stop; ++dd, ++i) {
      ...
      // std::cout << "sendIdx' " << dd << "   " << messages[dd][0] << "   " <<
      // chii << std::endl;
  ...
  if (eq(t1, received_t) && eq(t2, received_t2)) {
    ...
    //            std::endl; std::cout << "rec t = " << received_t << std::endl;
