...
/* size of bigs and elliptic curve points for memory allocation from stack or
 * heap */
...
#define MR_SIZE                                                          \
  (((sizeof(struct bigtype) + (MR_STATIC + 2) * sizeof(mr_utype)) - 1) / \
       MR_SL +                                                           \
   1) *                                                                  \
      MR_SL
...
#define MR_ESIZE \
  (((sizeof(epoint) + MR_BIG_RESERVE(2)) - 1) / MR_SL + 1) * MR_SL
#else
#define MR_ESIZE \
  (((sizeof(epoint) + MR_BIG_RESERVE(3)) - 1) / MR_SL + 1) * MR_SL
#endif
...
#define mr_size(n)                                                         \
  (((sizeof(struct bigtype) + ((n) + 2) * sizeof(mr_utype)) - 1) / MR_SL + \
   1) *                                                                    \
      MR_SL
#define mr_big_reserve(n, m) ((n)*mr_size(m) + MR_SL)

#define mr_esize_a(n) \
  (((sizeof(epoint) + mr_big_reserve(2, (n))) - 1) / MR_SL + 1) * MR_SL
...
#else
#define mr_esize(n) \
  (((sizeof(epoint) + mr_big_reserve(3, (n))) - 1) / MR_SL + 1) * MR_SL
#endif
...
#define MR_ECC_STORE_N \
  3 /* point store for ecn  variable point multiplication */
#define MR_ECC_STORE_2M \
  3 /* point store for ec2m variable point multiplication */
#define MR_ECC_STORE_N2 \
  3 /* point store for ecn2 variable point multiplication */
#else
...
/* If multi-addition is of m points, and s precomputed values are required, this
 * is max of m*s (=4.10?) */
...
#define MR_IN(N)                           \
  ...
  if (mr_mip->depth < MR_MAXDEPTH) {       \
    ...
    if (mr_mip->TRACER) mr_track(_MIPPO_); \
  }
#endif
...
extern mr_small imuldiv(mr_small, mr_small, mr_small, mr_small, mr_large,
                        mr_small *);
...
#ifndef MR_STATIC
extern BOOL ebrick2_init(_MIPT_ ebrick2 *, big, big, big, big, int, int, int,
                         int, int, int);
extern void ebrick2_end(ebrick2 *);
#else
extern void ebrick2_init(ebrick2 *, const mr_small *, big, big, int, int, int,
                         int, int, int);
#endif
...
extern void ecurve_double_add(_MIPT_ epoint *, epoint *, epoint *, epoint *,
                              big *, big *);
...
extern int ecn2_mul4_gls_v(_MIPT_ big *, int, ecn2 *, big *, ecn2 *, zzn2 *,
                           ecn2 *);
extern int ecn2_muln_engine(_MIPT_ int, int, int, int, big *, big *, big *,
                            big *, ecn2 *, ecn2 *, ecn2 *);
...
#ifndef MR_STATIC
extern BOOL ecn2_brick_init(_MIPT_ ebrick *, zzn2 *, zzn2 *, big, big, big, int,
                            int);
...
#ifdef MR_WIN64
#define muldvd(a, b, c, rp) \
  (*(rp) = _umul128((a), (b), &(tm)), *(rp) += (c), tm += (*(rp) < (c)), tm)
#define muldvd2(a, b, c, rp)                                          \
  (tr = _umul128((a), (b), &(tm)), tr += (*(c)), tm += (tr < (*(c))), \
   tr += (*(rp)), tm += (tr < (*(rp))), *(rp) = tr, *(c) = tm)
#endif
...
#ifdef MR_ITANIUM
#define muldvd(a, b, c, rp) \
  (tm = _m64_xmahu((a), (b), (c)), *(rp) = _m64_xmalu((a), (b), (c)), tm)
#define muldvd2(a, b, c, rp)                                               \
  (tm = _m64_xmalu((a), (b), (*(c))), *(c) = _m64_xmahu((a), (b), (*(c))), \
   tm += *(rp), *(c) += (tm < *(rp)), *(rp) = tm)
#endif
#endif
/*

SSE2 code. Works as for itanium - but in fact it is slower than the regular code
so not recommended Would require a call to emmintrin.h or xmmintrin.h, and an
__m128i variable tm to be declared in effected functions. But it works!

        #define muldvd(a,b,c,rp)
(tm=_mm_add_epi64(_mm_mul_epu32(_mm_cvtsi32_si128((a)),_mm_cvtsi32_si128((b))),_mm_cvtsi32_si128((c))),*(rp)=_mm_cvtsi128_si32(tm),_mm_cvtsi128_si32(_mm_shuffle_epi32(tm,_MM_SHUFFLE(3,2,0,1)))
) #define muldvd2(a,b,c,rp)
(tm=_mm_add_epi64(_mm_add_epi64(_mm_mul_epu32(_mm_cvtsi32_si128((a)),_mm_cvtsi32_si128((b))),_mm_cvtsi32_si128(*(c))),_mm_cvtsi32_si128(*(rp))),*(rp)=_mm_cvtsi128_si32(tm),*(c)=_mm_cvtsi128_si32(
_mm_shuffle_epi32(tm,_MM_SHUFFLE(3,2,0,1))  )
*/
...
extern unsigned int muldiv(unsigned int, unsigned int, unsigned int, unsigned
int, unsigned int *);
#pragma aux muldiv=                 \
       "mul     edx"                \
       "add     eax,ebx"            \
...
extern unsigned int muldvm(unsigned int, unsigned int, unsigned int, unsigned
int *);
#pragma aux muldvm=                 \
        "div     ebx"               \
        "mov     [ecx],edx"         \
...
extern unsigned int muldvd(unsigned int, unsigned int, unsigned int, unsigned
int *);
#pragma aux muldvd=                 \
        "mul     edx"               \
        "add     eax,ebx"           \
