...
void zzn2_mul(_MIPD_ zzn2 *x, zzn2 *y, zzn2 *w) {
  ...
  if (zzn2_iszero(x) || zzn2_iszero(y))
    zzn2_zero(w);
  else {
#ifdef MR_COUNT_OPS
    fpmq++;
    ...
    if (x->a->len != 0 && x->b->len != 0 && y->a->len != 0 && y->b->len != 0)
      ...
      if (mr_mip->qnr == -2) nres_modsub(_MIPP_ w->a, mr_mip->w2, w->a);
#ifndef MR_NO_LAZY_REDUCTION
    }
#endif
  }
...
void zzn2_inv(_MIPD_ zzn2 *w) {
  ...
  if (mr_mip->qnr == -2) nres_modadd(_MIPP_ mr_mip->w1, mr_mip->w2, mr_mip->w1);
...
void zzn2_timesi(_MIPD_ zzn2 *u) {
  ...
  if (mr_mip->qnr == -2) nres_modadd(_MIPP_ u->a, u->a, u->a);
...
void zzn2_txd(
    _MIPD_ zzn2
        *u) { /* divide w by t^2 where x^2-t is irreducible polynomial for ZZn4
