...
int egcd(_MIPD_ big x, big y,
         big z) { /* greatest common divisor z=gcd(x,y) by Euclids  *
  ...
  while (size(mr_mip->w2) != 0) {
    ...
    if (mr_mip->base == 0) {
      ...
      u = muldiv(mr_mip->w1->w[n - 1], mr_mip->base, mr_mip->w1->w[n - 2], m,
                 &sr);
      v = muldiv(mr_mip->w2->w[n - 1], mr_mip->base, mr_mip->w2->w[n - 2], m,
                 &sr);
    }
#endif
    /*   printf("u= %d v= %d\n",u,v);*/
