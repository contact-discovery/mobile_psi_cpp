...
static MR_WORD pack(const MR_BYTE *b) { /* pack bytes into a 32-bit Word */
  return ((MR_WORD)b[0] << 24) | ((MR_WORD)b[1] << 16) | ((MR_WORD)b[2] << 8) |
         (MR_WORD)b[3];
}
...
static void precompute(
    gcm *g, MR_BYTE *H) { /* precompute small 2k bytes gf2m table of x^n.H */
  int i, j;
  MR_WORD *last, *next, b;
...
static void gf2mul(gcm *g) { /* gf2m mul - Z=H*X mod 2^128 */
  ...
  for (i = 0; i < 128; i++) {
    b = (g->stateX[m] >> (--j)) & 1;
    if (b)
      for (k = 0; k < NB; k++) P[k] ^= g->table[i][k];
    if (j == 0) {
      j = 8;
      m++;
      if (m == 16) break;
...
static void gcm_wrap(gcm *g) { /* Finish off GHASH */
  int i, j;
  MR_WORD F[4];
  MR_BYTE L[16];
...
void gcm_init(gcm *g, int nk, char *key, int niv,
              char *iv) { /* iv size niv is usually 12 bytes (96 bits). AES key
                             size nk can be 16,24 or 32 bytes */
  int i;
  MR_BYTE H[16];
  for (i = 0; i < 16; i++) {
    H[i] = 0;
...
BOOL gcm_add_header(gcm *g, char *header,
                    int len) { /* Add some header. Won't be encrypted, but will
                                  be authenticated. len is length of header */
  int i, j = 0;
...
BOOL gcm_add_cipher(
    gcm *g, int mode, char *plain, int len,
    char *cipher) { /* Add plaintext to extract ciphertext, or visa versa,
                       depending on mode. len is length of plaintext/ciphertext.
                       Note this file combines GHASH() functionality with
...
void gcm_finish(gcm *g,
                char *tag) { /* Finish off GHASH and extract tag (MAC) */
  int i;
...
encrypted char N[100];   // IV - Initialisation vector char M[100];  //
Plaintext to be encrypted/authenticated char C[100];  // Ciphertext char P[100];
// Recovered Plaintext
