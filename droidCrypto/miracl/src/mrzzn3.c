...
BOOL zzn3_compare(zzn3 *x, zzn3 *y) {
  if (mr_compare(x->a, y->a) == 0 && mr_compare(x->b, y->b) == 0 &&
      mr_compare(x->c, y->c) == 0)
    return TRUE;
  return FALSE;
}
...
void zzn3_powq(_MIPD_ zzn3 *x,
               zzn3 *w) { /* sru - precalculated sixth root of unity */
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
  MR_IN(178)
  zzn3_copy(x, w);
  nres_modmult(_MIPP_ mr_mip->sru, mr_mip->sru,
               mr_mip->w1); /* cube root of unity */
...
void zzn3_smul(_MIPD_ zzn3 *x, big y, zzn3 *w) {
  ...
  MR_IN(184)
  if (size(x->a) != 0)
    nres_modmult(_MIPP_ x->a, y, w->a);
  else
    zero(w->a);
  if (size(x->b) != 0)
    nres_modmult(_MIPP_ x->b, y, w->b);
  else
    zero(w->b);
  if (size(x->c) != 0)
    nres_modmult(_MIPP_ x->c, y, w->c);
  else
    zero(w->c);
...
void zzn3_imul(_MIPD_ zzn3 *x, int y, zzn3 *w) {
  ...
  MR_IN(185)
  if (size(x->a) != 0)
    nres_premult(_MIPP_ x->a, y, w->a);
  else
    zero(w->a);
  if (size(x->b) != 0)
    nres_premult(_MIPP_ x->b, y, w->b);
  else
    zero(w->b);
  if (size(x->c) != 0)
    nres_premult(_MIPP_ x->c, y, w->c);
  else
    zero(w->c);
