...
static int euclid(_MIPD_ big x,
                  int num) { /* outputs next c.f. quotient from gcd(w5,w6) */
  mr_small sr, m;
#ifdef MR_FP
  mr_small dres;
#endif
  ...
  } else if (num == mr_mip->oldn || mr_mip->q < 0)
    return mr_mip->q;
  mr_mip->oldn = num;
  ...
  if (mr_mip->ndig == 1) {
    ...
    if (mr_mip->base == 0) {
      ...
      if (m == 0) {
        ...
        mr_mip->u = muldvm(mr_mip->w5->w[mr_mip->ndig - 1],
                           mr_mip->w5->w[mr_mip->ndig - 2], m, &sr);
        mr_mip->v = muldvm(mr_mip->w6->w[mr_mip->ndig - 1],
                           mr_mip->w6->w[mr_mip->ndig - 2], m, &sr);
      }
#endif
    } else {
      mr_mip->u = muldiv(mr_mip->w5->w[mr_mip->ndig - 1], mr_mip->base,
                         mr_mip->w5->w[mr_mip->ndig - 2], m, &sr);
      mr_mip->v = muldiv(mr_mip->w6->w[mr_mip->ndig - 1], mr_mip->base,
                         mr_mip->w6->w[mr_mip->ndig - 2], m, &sr);
    }
  }
  mr_mip->ku = mr_mip->u;
...
middle:
  forever { /* work only with most significant piece */
    if (mr_mip->last) {
      ...
      if (((mr_mip->v + mr_mip->c) == 0) || ((mr_mip->v + mr_mip->d) == 0))
        break;
...
void mround(_MIPD_ big num, big den,
            flash z) { /* reduces and rounds the fraction num/den into z */
  int s;
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
