 ...
 *
 *   This is a little slower for p=1 mod 4 primes, but its not time critical,
 * and more importantly it doesn't pull in the large powmod code into elliptic
 * curve programs It does require code from mrjack.c and mrlucas.c
 *
 *   If p=3 mod 4, then  sqrt(a)=a^[(p+1)/4] mod p. Note that for many elliptic
 * curves (p+1)/4 has very low hamming weight.
 *
 *   (was sqrt(a) = V_{(p+1)/4}(a+1/a,1)/(1+1/a))
 *
 *   Mueller's method is also very simple, uses very little memory, and it works
 * just fine for p=1 mod 8 primes (for example the "annoying" NIST modulus
 * 2^224-2^96+1) Also doesn't waste time on non-squares, as a jacobi test is
 * done first
 *
 *   If you know that the prime is 3 mod 4, and you know that x is almost
 * certainly a QR then the jacobi-dependent code can be deleted with some space
 * savings.
 *
...
BOOL nres_sqroot(_MIPD_ big x,
                 big w) { /* w=sqrt(x) mod p. This depends on p being prime! */
  int t, js;
  ...
  if (js ==
      1) { /* 3 mod 4 primes - do a quick and dirty sqrt(x)=x^(p+1)/4 mod p */
    ...
    nres_lucas(_MIPP_ mr_mip->w3, mr_mip->w10, w,
               w); /* heavy lifting done here */
    if (t != 1) {
