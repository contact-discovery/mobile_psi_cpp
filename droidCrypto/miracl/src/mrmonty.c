...
mr_small prepare_monty(_MIPD_ big n) { /* prepare Montgomery modulus */
  ...
  if (mr_mip->ACTIVE) {
    ...
    mr_mip->w6->w[n->len] = 1;
    if (invmodp(_MIPP_ n, mr_mip->w6, mr_mip->w14) != 1) { /* problems */
      mr_berror(_MIPP_ MR_ERR_BAD_MODULUS);
      MR_OUT
      return (mr_small)0;
    }
    ...
    if (invmodp(_MIPP_ mr_mip->w15, mr_mip->w6, mr_mip->w14) !=
        1) { /* problems */
      mr_berror(_MIPP_ MR_ERR_BAD_MODULUS);
      MR_OUT
      return (mr_small)0;
  ...
  if (mr_mip->ACTIVE) {
    ...
    magic *= base;
    ASM {
      finit fldcw WORD PTR control fld QWORD PTR ndash fld1 fld QWORD PTR base
          fdiv fld QWORD PTR magic
    }
  }
#endif
...
#if INLINE_ASM == 1
      ASM cld ASM mov cx, rn ASM mov si, i ASM shl si,
          1
#ifdef MR_LMM
          ASM push ds ASM push es ASM les bx,
          DWORD PTR w0g ASM add bx, si ASM mov ax,
          es : [bx] ASM mul WORD PTR ndash ASM mov di,
               ax ASM lds si,
               DWORD PTR mg
#else
          ASM mov bx,
          w0g ASM add bx, si ASM mov ax, [bx] ASM mul WORD PTR ndash ASM mov di,
          ax ASM mov si,
          mg
#endif
                       ASM push bp ASM xor
                   bp,
               bp m1 : ASM lodsw ASM mul di ASM add ax,
                       bp ASM adc dx,
                       0
#ifdef MR_LMM
                       ASM add es : [bx],
...
#endif
                                        ASM adc dx,
                                    0 ASM inc bx ASM inc bx ASM mov bp,
                                    dx ASM loop m1

                                        ASM pop bp ASM mov ax,
                                    delay_carry
#ifdef MR_LMM
                                        ASM add es : [bx],
                                                     ax ASM mov ax,
...
#ifdef MR_LMM
            ASM push ds ASM push es ASM les bx,
            DWORD PTR w0g ASM add bx,
            si ASM mov eax,
            es : [bx] ASM mul DWORD PTR ndash ASM mov edi,
                 eax ASM lds si,
                 DWORD PTR mg
#else
                  ASM mov bx,
                  w0g ASM add bx,
                  si ASM mov eax,
                  [bx] ASM mul DWORD PTR ndash ASM mov edi,
                  eax ASM mov si,
                  mg
#endif
                         ASM push ebp ASM xor
                     ebp,
                 ebp m1 : ASM lodsd ASM mul edi ASM add eax,
                          ebp ASM adc edx,
                          0
#ifdef MR_LMM
                          ASM add es : [bx],
          ...
          : [bx],
            ...
            w0g ASM add ebx,
            esi ASM mov eax,
            [ebx] ASM mul DWORD PTR ndash ASM mov edi,
            eax ASM mov esi,
            mg ASM sub ebx,
            esi ASM sub ebx,
            4 ASM push ebp ASM xor ebp,
            ebp m1
          : ASM mov eax,
            [esi] ASM add esi,
            4 ASM mul edi ASM add eax,
            ebp ASM mov ebp,
            [esi + ebx] ASM adc edx,
            0 ASM add ebp,
            eax ASM adc edx,
            0 ASM mov[esi + ebx],
            ebp ASM dec ecx ASM mov ebp,
            edx ASM jnz m1

                ASM pop ebp ASM mov eax,
            delay_carry ASM add[esi + ebx + 4],
            eax ASM mov eax,
            0 ASM adc eax,
            0 ASM add[esi + ebx + 4],
            edx ASM adc eax,
...
#else
#ifdef MR_FP_ROUNDING
        carry = imuldiv(modulus->w[j], m, w0->w[i + j] + carry, mr_mip->base,
                        mr_mip->inverse_base, &w0->w[i + j]);
#else
        carry = muldiv(modulus->w[j], m, w0->w[i + j] + carry, mr_mip->base,
                       &w0->w[i + j]);
#endif
#endif
      }
...
void nres_complex(_MIPD_ big a, big b, big r, big i) {
  ...
  if (mr_mip->NO_CARRY &&
      mr_mip->qnr == -1) { /* if modulus is small enough we can ignore carries,
                              and use simple addition and subtraction */
    /* recall that Montgomery reduction can cope as long as product is less than
     * pR */
#ifdef MR_COMBA
#ifdef MR_COUNT_OPS
    fpa += 3;
    ...
    if (mr_mip->qnr == -2) nres_modsub(_MIPP_ mr_mip->w2, b, mr_mip->w2);

    nres_modmult(_MIPP_ a, b, i);
    nres_modmult(_MIPP_ mr_mip->w1, mr_mip->w2, r);

    if (mr_mip->qnr == -2) nres_modadd(_MIPP_ r, i, r);
...
Lazy reduction technique for zzn2 multiplication - competitive if Reduction is
more expensive that Multiplication. This is true for pairing-based crypto. Note
that Lazy reduction can also be used with Karatsuba! Uses w1, w2, w5, and w6.
...
void nres_lazy(_MIPD_ big a0, big a1, big b0, big b1, big r, big i) {
  ...
  if (mr_mip->NO_CARRY &&
    ...
    nres_double_modadd(_MIPP_ mr_mip->w0, mr_mip->w5,
                       mr_mip->w6); /* w6 =  a0.b0+a1.b1 */
    if (mr_mip->qnr == -2)
  ...
  nres_double_modsub(_MIPP_ mr_mip->w0, mr_mip->w5,
                     mr_mip->w0); /* r = a0.b0+D.a1.b1 */
  ...
  if (mr_mip->ACTIVE) {
    ...
    if (mr_mip->ACTIVE) {
      ...
      multiply(_MIPP_ mr_mip->w1, mr_mip->w2,
               mr_mip->w0); /* w0=(a0+a1)*(b0+b1) */
#ifdef MR_COMBA
    }
#endif
#ifdef MR_KCM
  ...
  if (mr_mip->NO_CARRY && mr_mip->qnr == -1) {
    ...
    nres_double_modsub(_MIPP_ mr_mip->w0, mr_mip->w6,
                       mr_mip->w0); /* (a0+a1)*(b0+b1) - w6 */
...
void nres_double_modadd(_MIPD_ big x, big y, big w) {
  ...
  if (mr_mip->ACTIVE) {
    ...
    if (mr_compare(w, mr_mip->pR) >= 0) mr_psub(_MIPP_ w, mr_mip->pR, w);
...
void nres_modadd(_MIPD_ big x, big y, big w) { /* modular addition */
  ...
  if (mr_mip->ACTIVE) {
    ...
    if (mr_compare(w, mr_mip->modulus) >= 0)
      mr_psub(_MIPP_ w, mr_mip->modulus, w);
...
int nres_moddiv(_MIPD_ big x, big y,
                big w) { /* Modular division using n-residues w=x/y mod n */
  int gcd;
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
...
void nres_premult(_MIPD_ big x, int k,
                  big w) { /* multiply n-residue by small ordinary integer */
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
  int sign = 0;
...
void nres_modmult(
    _MIPD_ big x, big y,
    big w) { /* Modular multiplication using n-residues w=x*y mod n */
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
  ...
  if (mr_mip->ACTIVE) {
    if (x == y)
      comba_square(x, mr_mip->w0);
    else
      comba_mult(x, y, mr_mip->w0);
    comba_redc(_MIPP_ mr_mip->w0, w);
  } else {
#endif
#ifdef MR_KCM
    if (mr_mip->ACTIVE) {
      if (x == y)
        kcm_sqr(_MIPP_ x, mr_mip->w0);
      else
        kcm_mul(_MIPP_ x, y, mr_mip->w0);
      ...
      if (mr_mip->ACTIVE) {
        if (x == y)
          fastmodsquare(_MIPP_ x, w);
        else
          fastmodmult(_MIPP_ x, y, w);
      } else {
#endif
        if (mr_mip->ERNUM) return;
...
BOOL nres_double_inverse(_MIPD_ big x, big y, big w,
                         big z) { /* find y=1/x mod n and z=1/w mod n */
...
BOOL nres_multi_inverse(_MIPD_ int m, big *x,
                        big *w) { /* find w[i]=1/x[i] mod n, for i=0 to m-1 *
  ...
  copy(x[0], w[1]);
  for (i = 2; i < m; i++) nres_modmult(_MIPP_ w[i - 1], x[i - 1], w[i]);

  nres_modmult(_MIPP_ w[m - 1], x[m - 1],
               mr_mip->w6); /* y=x[0]*x[1]*x[2]....x[m-1] */
  if (size(mr_mip->w6) == 0) {
    mr_berror(_MIPP_ MR_ERR_DIV_BY_ZERO);
    MR_OUT
    return FALSE;
...
void ecurve_init(_MIPD_ big a, big b, big p,
                 int type) { /* Initialize the active ecurve    *
  ...
  if (mr_abs(mr_mip->Asize) == MR_TOOBIG) {
    if (mr_mip->Asize >=
        0) { /* big positive number - check it isn't minus something small */
      copy(a, mr_mip->w1);
  ...
  if (type == MR_BEST)
    mr_mip->coord = MR_PROJECTIVE;
  else
    mr_mip->coord = type;
#else
  if (type == MR_PROJECTIVE) mr_berror(_MIPP_ MR_ERR_NOT_SUPPORTED);
#endif
#endif
  MR_OUT
  return;
