...
void build(_MIPD_ flash x,
           int (*gen)(_MIPT_ big, int)) { /* Build x from its regular c.f. *
                                           * generated by gen()            */
  ...
  ex = mr_mip->base - 1;
  if (mr_mip->nib == mr_mip->workprec)
    prc = mr_mip->nib;
  else
    prc = mr_mip->workprec + 1;
  while (!mr_mip->ERNUM && q >= 0) {
    ...
    if (lz >= prc - 1) { /* nearly finished - so be careful not to overshoot */
      if (mr_mip->base == 0) {
        ...
        if (st == 0)
          ...
          ex1 = muldvm((mr_small)1, (mr_small)0, st, &sr);
        st = mr_mip->w4->w[lw4 - 1] + 1;
        if (st == 0)
          ex2 = 1;
        else
          ex2 = muldvm((mr_small)1, (mr_small)0, st, &sr);
#endif
      } else {
    ...
    forever {
      ...
      if (q < 0 ||
          q >= MR_TOOBIG /
                   mr_abs(d)) { /* there could be more.... *** V3.21 mod *** */
        last = FALSE;
        break;
      }
      rm = b - q * d;
  ...
  if (fit(mr_mip->w2, mr_mip->w4, mr_mip->nib))
    fpack(_MIPP_ mr_mip->w2, mr_mip->w4, x);
  else
    fpack(_MIPP_ mr_mip->w1, mr_mip->w3, x);
  negify(x, x);
