...
BOOL ebrick_init(
    _MIPD_ ebrick *B, big x, big y, big a, big b, big n, int window,
    int nb) { /* Uses Montgomery arithmetic internally              *
  ...
  for (j = 0; j < t; j++) ecurve_double(_MIPP_ w);
  k = 1;
  ...
  bptr = 0;
  B->table =
      (mr_small *)mr_alloc(_MIPP_ 2 * len * (1 << window), sizeof(mr_small));
...
/* use precomputated table in ROM - see romaker.c to create the table, and
   ecdhp.c for an example of use */

void ebrick_init(ebrick *B, const mr_small *rom, big a, big b, big n,
                 int window, int nb) {
  B->table = rom;
