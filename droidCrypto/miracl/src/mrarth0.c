...
void mr_padd(_MIPD_ big x, big y, big z) { /*  add two  big numbers, z=x+y where
  ...
  if (mr_mip->base == 0) {
    ...
    for (; i < lz && carry > 0; i++) { /* add by columns to the length of larger
                                          number (if there is a carry) */
      psum = gx[i] + gy[i] + carry;
      if (psum > gx[i])
        carry = 0;
      else if (psum < gx[i])
...
void mr_psub(_MIPD_ big x, big y,
             big z) { /*  subtract two big numbers z=x-y      *
...
static void mr_select(_MIPD_ big x, int d, big y,
                      big z) { /* perform required add or subtract operation */
...
void add(_MIPD_ big x, big y,
         big z) { /* add two signed big numbers together z=x+y */
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
...
void subtract(_MIPD_ big x, big y,
              big z) { /* subtract two big signed numbers z=x-y */
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
  if (mr_mip->ERNUM) return;
...
void decr(_MIPD_ big x, int n,
          big z) { /* subtract int from big number: z=x-n */
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
  if (mr_mip->ERNUM) return;
