...
BOOL epoint_set(
    _MIPD_ big x, big y, int cb,
    epoint *p) { /* initialise a point on active ecurve            *
...
void epoint_getxyz(_MIPD_ epoint *p, big x, big y,
                   big z) { /* get (x,y,z) coordinates */
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
...
int epoint_get(
    _MIPD_ epoint *p, big x,
    big y) { /* Get point co-ordinates in affine, normal form       *
...
BOOL epoint_multi_norm(
    _MIPD_ int m, big *work,
    epoint **p) { /* Normalise an array of points of length m<MR_MAX_M_T_S -
                     requires a workspace array of length m */
  ...
  for (i = 0; i < m; i++) {
    if (p[i]->marker == MR_EPOINT_NORMALIZED)
      w[i] = mr_mip->one;
    else
      w[i] = p[i]->Z;
    if (p[i]->marker == MR_EPOINT_INFINITY) {
      inf = TRUE;
      break;
...
void ecurve_double_add(_MIPD_ epoint *a, epoint *b, epoint *c, epoint *d,
                       big *s1, big *s2) {
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
  ...
  if (mr_mip->coord == MR_AFFINE) {
    ...
    if (a == b ||
        (mr_compare(a->X, b->X) == 0 && mr_compare(a->Y, b->Y) == 0)) {
    ...
    if (c == d ||
        (mr_compare(c->X, d->X) == 0 && mr_compare(c->Y, d->Y) == 0)) {
    ...
    nres_double_inverse(_MIPP_ mr_mip->w10, mr_mip->w10, mr_mip->w11,
                        mr_mip->w11);
...
void ecurve_multi_add(
    _MIPD_ int m, epoint **x,
    epoint **w) { /* adds m points together simultaneously, w[i]+=x[i] */
  int i, *flag;
  big *A, *B, *C;
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
  ...
  if (mr_mip->coord == MR_AFFINE) { /* this can be done faster */
    ...
    for (i = 0; i < m; i++) {
      ...
      C[i] = mirvar(_MIPP_ 0);
      flag[i] = 0;
      if (mr_compare(x[i]->X, w[i]->X) == 0 &&
          mr_compare(x[i]->Y, w[i]->Y) == 0) { /* doubling */
...
void ecurve_double(_MIPD_ epoint *p) { /* double epoint on active ecurve */
  ...
  if (p->marker == MR_EPOINT_NORMALIZED)
    ...
    nres_modmult(_MIPP_ p->Z, p->Y, p->Z);
  ...
  */
  ...
  Observe that when finished w8 contains the line slope, w7 has 2y^2 and w6 has
  z^2 This is useful for calculating line functions in pairings
...
static BOOL ecurve_padd(
    _MIPD_ epoint *p,
    epoint
        *pa) { /* primitive add two epoints on the active ecurve - pa+=p;   *
...
BOOL epoint_comp(_MIPD_ epoint *a, epoint *b) {
  ...
  if (b->marker == MR_EPOINT_INFINITY) return FALSE;

#ifndef MR_AFFINE_ONLY
  if (mr_mip->coord == MR_AFFINE) {
#endif
    if (mr_compare(a->X, b->X) == 0 && mr_compare(a->Y, b->Y) == 0)
      result = TRUE;
    else
      result = FALSE;
    return result;
  ...
  mem1 = (char *)memalloc(_MIPP_ MR_ECC_STORE_N);
    ...
    for (i = nb - 1; i >= 1;) { /* add/subtract */
      ...
      n = mr_naf_window(_MIPP_ mr_mip->w9, mr_mip->w10, i, &nbs, &nzs,
                        MR_ECC_STORE_N);
      for (j = 0; j < nbs; j++) ecurve_double(_MIPP_ pt);
      if (n > 0) {
...
void ecurve_multn(
    _MIPD_ int n, big *y, epoint **x,
    epoint *w) { /* pt=e[o]*p[0]+e[1]*p[1]+ .... e[n-1]*p[n-1]   */
  int i, j, k, m, nb, ea;
  epoint **G;
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
...
static BOOL ecurve_add_sub(_MIPD_ epoint *P, epoint *Q, epoint *PP,
                           epoint *PM) {
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
#endif
...
void ecurve_mult2(_MIPD_ big e, epoint *p, big ea, epoint *pa,
                  epoint *pt) { /* pt=e*p+ea*pa; */
  int e1, h1, e2, h2, bb;
  epoint *p1, *p2, *ps[2];
#ifdef MR_STATIC
  ...
  ecurve_add_sub(_MIPP_ p1, p2, ps[0],
                 ps[1]); /* only one inversion! ps[0]=p1+p2, ps[1]=p1-p2 */

  mr_jsf(_MIPP_ mr_mip->w9, mr_mip->w12, mr_mip->w10, mr_mip->w9, mr_mip->w13,
         mr_mip->w12);
  ...
  */
  ...
  if (mr_mip->base == mr_mip->base2) {
    ...
    if (mr_compare(mr_mip->w10, mr_mip->w13) >= 0)
      bb = logb2(_MIPP_ mr_mip->w10) - 1;
    else
      bb = logb2(_MIPP_ mr_mip->w13) - 1;
    ...
    while (bb >= 0) /* for the simple NAF, this should be 1 */
    {
      ...
      if (e1 != h1) {
        if (e2 == h2) {
          if (h1 == 1)
            ecurve_add(_MIPP_ p1, pt);
          else
            ecurve_sub(_MIPP_ p1, pt);
        } else {
          if (h1 == 1) {
            if (h2 == 1)
              ecurve_add(_MIPP_ ps[0], pt);
            else
              ecurve_add(_MIPP_ ps[1], pt);
          } else {
            if (h2 == 1)
              ecurve_sub(_MIPP_ ps[1], pt);
            else
              ecurve_sub(_MIPP_ ps[0], pt);
          }
        }
      } else if (e2 != h2) {
        if (h2 == 1)
          ecurve_add(_MIPP_ p2, pt);
        else
          ecurve_sub(_MIPP_ p2, pt);
      }
      bb -= 1;
    }
#ifndef MR_ALWAYS_BINARY
    ...
    if (mr_compare(mr_mip->w10, mr_mip->w13) >= 0)
      ...
      expb2(_MIPP_ logb2(_MIPP_ mr_mip->w13) - 1, mr_mip->w11);
...
BOOL epoint_set(
  ...
  valid = FALSE;
  nres(_MIPP_ x, p->X);
  if (x != y) { /* Check directly that x^2+Ay^2 == x^2.y^2+B */
    nres(_MIPP_ y, p->Y);
    nres_modmult(_MIPP_ p->X, p->X, mr_mip->w1);
    nres_modmult(_MIPP_ p->Y, p->Y, mr_mip->w2);
...
static BOOL ecurve_padd(
  ...
  nres_modmult(_MIPP_ mr_mip->w1, mr_mip->w2,
               mr_mip->w1); /* I=(X1+Y1)(X2+Y2) */
  if (p->marker != MR_EPOINT_NORMALIZED) {
    if (pa->marker == MR_EPOINT_NORMALIZED)
      copy(p->Z, pa->Z);
    else
      nres_modmult(_MIPP_ p->Z, pa->Z, pa->Z); /* z = A = Z1*Z2 */
  } else {
  ...
  nres_modsub(_MIPP_ mr_mip->w1, pa->X, mr_mip->w1);
  nres_modsub(_MIPP_ mr_mip->w1, pa->Y,
              mr_mip->w1); /* I=(X1+Y1)(X2+Y2)-C-D =X1*Y2+Y1*X2 */
...
void ecurve_multn(
    _MIPD_ int n, big *y, epoint **x,
    epoint *w) { /* pt=e[0]*p[0]+e[1]*p[1]+ .... e[n-1]*p[n-1]   */
  int i, j, k, m, nb, ea;
  epoint **G;
#ifdef MR_OS_THREADS
  miracl *mr_mip = get_mip();
...
static BOOL ecurve_add_sub(_MIPD_ epoint *P, epoint *Q, epoint *PP,
  ...
  if (P->marker == MR_EPOINT_NORMALIZED) {
    ...
    if (Q->marker == MR_EPOINT_NORMALIZED)
      ...
      nres_modmult(_MIPP_ P->Z, Q->Z, mr_mip->w1); /* w1 = A = Z1*Z2 */
  }
