...
static void shs_transform(sha512 *sh) { /* basic transformation step */
  ...
  int j;
  for (j = 16; j < 80; j++)
    sh->w[j] = theta1(sh->w[j - 2]) + sh->w[j - 7] + theta0(sh->w[j - 15]) +
               sh->w[j - 16];
...
void shs512_hash(sha512 *sh,
                 char hash[64]) { /* pad message and finish - supply digest */
  int i;
...
void shs384_hash(sha384 *sh,
                 char hash[48]) { /* pad message and finish - supply digest */
  int i;
  mr_unsign64 len0, len1;
  len0 = sh->length[0];
  len1 = sh->length[1];
