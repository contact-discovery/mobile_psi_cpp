...
void gprime(_MIPD_ int maxp) { /* generate all primes less than maxp into global
                                  PRIMES */
  char *sv;
  int pix, i, k, prime;
#ifdef MR_OS_THREADS
  ...
  pix = 1;
  for (i = 0; i < maxp; i++) sv[i] = TRUE;
  for (i = 0; i < maxp; i++)
    if (sv[i]) { /* using sieve of Eratosthenes */
      prime = i + i + 3;
      for (k = i + prime; k < maxp; k += prime) sv[k] = FALSE;
      pix++;
    }
  if (mr_mip->PRIMES != NULL) mr_free(mr_mip->PRIMES);
...
void gprime(_MIPD_ int maxp) { /* dummy - does nothing */
}
...
#endif
  for (i = 0; mr_mip->PRIMES[i] != 0;
       i++) { /* test for divisible by first few primes */
...
BOOL isprime(
    _MIPD_ big
        x) { /*  test for primality (probably); TRUE if x is prime. test done
              * NTRY * times; chance of wrong identification << (1/4)^NTRY. Note
              * however * that this is an extreme upper bound. For example for a
              * 100 digit  * "prime" the chances of false witness are actually <
              * (.00000003)^NTRY * See Kim & Pomerance "The probability that a
              * random probable prime * is Composite", Math. Comp. October 1989
              * pp.721-741                * The value of NTRY is now adjusted
              * internally to account for this. */
  int j, k, n, r, times, d;
  ...
  for (n = 1; n <= times; n++) { /* perform test times times */
    ...
    while ((j > 0 || size(mr_mip->w9) != 1) &&
      ...
      j++;
      if ((j > 1 && size(mr_mip->w9) == 1) ||
          j == k) { /* definitely not prime */
        MR_OUT
        return FALSE;
      }
...
BOOL nxprime(_MIPD_ big w, big x) { /*  find next highest prime from w using *
  ...
  if (subdiv(_MIPP_ x, 2, mr_mip->w1) == 0)
    incr(_MIPP_ x, 1, x);
  else
    incr(_MIPP_ x, 2, x);
...
BOOL nxsafeprime(_MIPD_ int type, int subset, big w,
                 big p) { /* If type=0 finds next highest "safe" prime p >= w *
  ...
  if (subset == 0) {
    rem = rem % 4;
    if (type == 0)
      incr(_MIPP_ p, 3 - rem, p);
    else {
      if (rem > 1)
        incr(_MIPP_ p, 5 - rem, p);
      else
        incr(_MIPP_ p, 1 - rem, p);
    }
    increment = 4;
  } else {
    if (subset == 1) {
      if (type == 0) {
        if (rem > 3)
          incr(_MIPP_ p, 11 - rem, p);
        else
          incr(_MIPP_ p, 3 - rem, p);
      } else {
        if (rem > 1)
          incr(_MIPP_ p, 9 - rem, p);
        else
          incr(_MIPP_ p, 1 - rem, p);
      }
    } else {
      if (type == 0)
        incr(_MIPP_ p, 7 - rem, p);
      else {
        if (rem > 5)
          incr(_MIPP_ p, 13 - rem, p);
        else
          incr(_MIPP_ p, 5 - rem, p);
      }
    }
  ...
  if (type == 0)
    decr(_MIPP_ p, 1, mr_mip->w10);
  else
    incr(_MIPP_ p, 1, mr_mip->w10);
  ...
  forever {
    ...
    } while (!trial_division(_MIPP_ p, p) ||
             !trial_division(_MIPP_ mr_mip->w10, mr_mip->w10));
