...
REccNumber::REccNumber(const int32_t &val) {
  init();
  *this = val;
}
...
REccNumber::REccNumber(REllipticCurve &) { init(); }
...
REccNumber::REccNumber(REllipticCurve &, const int32_t &val) {
  init();
  *this = val;
}

REccNumber::~REccNumber() { bn_clean(*this); }
...
REccNumber &REccNumber::operator++() { return *this += 1; }
...
REccNumber &REccNumber::operator*=(int i) {
  if (i < 0) return *this *= REccNumber(i);
...
bool REccNumber::operator==(const int &cmp) const {
  if (cmp < 0) return *this == REccNumber(cmp);
...
bool REccNumber::operator!=(const int &cmp) const { return !(*this == cmp); }
...
bool REccNumber::operator<(const REccNumber &cmp) const { return cmp > *this; }
...
bool REccNumber::isPrime() const { return bn_is_prime(*this); }

bool REccNumber::iszero() const { return bn_is_zero(*this); }
...
REccNumber operator-(const REccNumber &v) { return v.negate(); }

REccNumber operator+(int i, const REccNumber &v) {
  if (i < 0) return REccNumber(i) + v;
...
REccNumber operator-(int i, const REccNumber &v) { return i + v.negate(); }
...
REccNumber operator*(int i, const REccNumber &v) { return v * i; }
REccNumber operator*(const REccNumber &v, const REccNumber &i) {
  REccNumber r;
  bn_mul(r, v, i);
...
REccNumber operator/(int i, const REccNumber &v) { return i * v.inverse(); }
REccNumber operator/(const REccNumber &i, const REccNumber &v) {
  return i * v.inverse();
}
...
std::ostream &operator<<(std::ostream &out, const REccPoint &val) {
  ...
  auto print = [radix](std::ostream &out, const fp_t &c) {
    ...
    if (int64_t(buff.size()) < int64_t(fp_size_str(c, radix))) {
      out << std::string("buff.size() ") << buff.size();
      out << std::string(" fp_size_str ") << fp_size_str(c, radix)
          << std::string("\n");
...
bool REccPoint::iszero() const { return ep_is_infty(*this); }
...
uint64_t REccNumber::sizeDigits() const { return bn_size_raw(modulus()); }

uint64_t REccNumber::sizeBytes() const { return bn_size_bin(modulus()); }

void REccNumber::toBytes(uint8_t *dest) const {
  bn_write_bin(dest, static_cast<int>(sizeBytes()), *this);
  if (GSL_UNLIKELY(err_get_code()))
    throw std::runtime_error("Relic write error " LOCATION);
}

void REccNumber::fromBytes(const uint8_t *src) {
...
void REccNumber::randomize(PRNG &prng) {
  std::vector<uint8_t> buff(sizeBytes() + 5);
