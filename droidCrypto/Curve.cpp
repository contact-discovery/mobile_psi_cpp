...
EllipticCurve::EllipticCurve(const Ecc2mParams &params, const block &seed)
    : mMiracl(nullptr), BA(nullptr), BB(nullptr), mOrder(nullptr) {
...
void EllipticCurve::setParameters(const EccpParams &params) {
  ...
  ecurve_init(mMiracl, BA, BB, mFieldPrime->mVal,
              // MR_AFFINE
              MR_PROJECTIVE);
...
void EllipticCurve::setPrng(const block &seed) {
  mPrng.SetSeed(seed);
  irand(mMiracl, (int)mPrng.get<uint32_t>());
}
...
void EllipticCurve::setParameters(const Ecc2mParams &params) {
  ...
  ecurve2_init(mMiracl, params.m, params.a, params.b, params.c, BA, BB, false,
               MR_PROJECTIVE);
...
miracl &EllipticCurve::getMiracl() const { return *mMiracl; }

const EllipticCurve::Point &EllipticCurve::getGenerator() const {
  return mG[0];
}

const std::vector<EccPoint> &EllipticCurve::getGenerators() const { return mG; }

const EccNumber &EllipticCurve::getOrder() const { return *mOrder; }

const EccNumber &EllipticCurve::getFieldPrime() const { return *mFieldPrime; }

uint64_t EllipticCurve::bitCount() const {
  if (mIsPrimeField)
...
EccPoint::EccPoint(EllipticCurve &curve)
    : mVal(nullptr),
      mMem(nullptr),
      mCurve(&curve)
...
EccPoint::EccPoint(EllipticCurve &curve, const EccPoint &copy)
    : mVal(nullptr), mMem(nullptr), mCurve(&curve) {
  init();
...
EccPoint::EccPoint(const EccPoint &copy)
    : mVal(nullptr), mMem(nullptr), mCurve(copy.mCurve) {
  init();
...
EccPoint::EccPoint(EccPoint &&move)
    : mVal(move.mVal), mMem(move.mMem), mCurve(move.mCurve) {
  move.mVal = nullptr;
  move.mMem = nullptr;
}

EccPoint::~EccPoint() {
  if (mMem) ecp_memkill(mCurve->mMiracl, mMem, 0);
}

EccPoint &EccPoint::operator=(const EccPoint &copy) {
...
EccPoint &EccPoint::operator+=(const EccPoint &addIn) {
#ifndef NDEBUG
  if (mCurve != addIn.mCurve)
    throw std::runtime_error("curves instances must match.");
#endif
...
EccPoint &EccPoint::operator-=(const EccPoint &subtractIn) {
#ifndef NDEBUG
  if (mCurve != subtractIn.mCurve)
    throw std::runtime_error("curves instances must match.");
#endif
  if (mCurve->mIsPrimeField) {
...
EccPoint &EccPoint::operator*=(const EccNumber &multIn) {
#ifndef NDEBUG
  if (mCurve != multIn.mCurve)
    throw std::runtime_error("curves instances must match.");
#endif
...
EccPoint EccPoint::operator+(const EccPoint &addIn) const {
#ifndef NDEBUG
  if (mCurve != addIn.mCurve)
    throw std::runtime_error("curves instances must match.");
#endif
...
EccPoint EccPoint::operator-(const EccPoint &subtractIn) const {
  EccPoint temp(*this);
...
EccPoint EccPoint::operator*(const EccNumber &multIn) const {
  EccPoint temp(*this);
...
bool EccPoint::operator==(const EccPoint &cmp) const {
#ifndef NDEBUG
  if (mCurve != cmp.mCurve)
    throw std::runtime_error("curves instances must match.");
#endif
  if (mCurve->mIsPrimeField) {
...
bool EccPoint::operator!=(const EccPoint &cmp) const { return !(*this == cmp); }

uint64_t EccPoint::sizeBytes() const {
  return (mCurve->bitCount() + 7) / 8 + 1;
}

void EccPoint::toBytes(uint8_t *dest) const {
  ...
  big_to_bytes(mCurve->mMiracl, (int)sizeBytes() - 1, varX, (char *)dest + 1,
               true);

  mirkill(varX);
}

void EccPoint::fromBytes(const uint8_t *src) {
  big varX = mirvar(mCurve->mMiracl, 0);
...
void EccPoint::setCurve(EllipticCurve &curve) { mCurve = &curve; }
...
EccNumber::EccNumber(const EccNumber &num) : mVal(nullptr), mCurve(num.mCurve) {
  init();

  *this = num;
}

EccNumber::EccNumber(EccNumber &&num) : mVal(num.mVal), mCurve(num.mCurve) {
  num.mVal = nullptr;
}

EccNumber::EccNumber(EllipticCurve &curve) : mVal(nullptr), mCurve(&curve) {
  init();
}

EccNumber::EccNumber(EllipticCurve &curve, const EccNumber &copy)
    : mVal(nullptr), mCurve(&curve) {
  init();
  *this = copy;
}
...
EccNumber::EccNumber(EllipticCurve &curve, const int32_t &val)
    : mVal(nullptr), mCurve(&curve) {
  init();
  *this = val;
}
...
EccNumber EccNumber::inverse() const {
  if (mCurve->mIsPrimeOrder == false) throw std::runtime_error(LOCATION);

  EccNumber ret(*this);

  // ret = ret ^ -1 mod order
  xgcd(mCurve->mMiracl, ret.mVal, mCurve->mOrder->mVal, ret.mVal, ret.mVal,
       ret.mVal);
...
bool EccNumber::operator==(const int &cmp) const { return cmp == *this; }

bool EccNumber::operator!=(const EccNumber &cmp) const {
  return !(*this == cmp);
}

bool EccNumber::operator!=(const int &cmp) const { return !(*this == cmp); }
...
uint64_t EccNumber::sizeBytes() const { return (mCurve->bitCount() + 7) / 8; }

void EccNumber::toBytes(uint8_t *dest) const {
  big_to_bytes(mCurve->mMiracl, (int)sizeBytes(), mVal, (char *)dest, true);
}

void EccNumber::fromBytes(const uint8_t *src) {
...
void EccNumber::init() { mVal = mirvar(mCurve->mMiracl, 0); }
...
void EccNumber::reduce() {
  ...
  if (*this >= mCurve->getOrder()) {
    ...
    divide(mCurve->mMiracl, mVal, mCurve->getOrder().mVal,
...
EccBrick::EccBrick(const EccPoint &copy) : mCurve(copy.mCurve) {
  bool result = 0;
  // big x, y;
  if (mCurve->mIsPrimeField) {
    ...
    result = 0 < ebrick_init(mCurve->mMiracl, &mBrick, x, y, mCurve->BA,
                             mCurve->BB, mCurve->getFieldPrime().mVal, 8,
    ...
    result = 0 < ebrick2_init(mCurve->mMiracl, &mBrick2, copy.mVal->X,
                              copy.mVal->Y, mCurve->BA, mCurve->BB,
                              mCurve->mEcc2mParams.m, mCurve->mEcc2mParams.a,
                              mCurve->mEcc2mParams.b, mCurve->mEcc2mParams.c, 8,
...
EccBrick::EccBrick(EccBrick &&copy)
    : mBrick2(copy.mBrick2), mCurve(copy.mCurve) {}
